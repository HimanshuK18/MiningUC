...
module.exports = function (app) {
    ...
    app.get('/users', ensureAuthorized, function (req, res) {
        var UData = {AllUsers:{},UserTypes: {},ProjectTypes:{}};
        ...
        db.Projects.find(function (err, docsp) {
            UData.ProjectTypes = docsp;
            res.json(UData);
        });
        });
    });
    ...
    var id = req.params.id;
       var db = mongojs('mongodb://127.0.0.1:27017/MiningDB', ['MiningUsers']);
        db.MiningUsers.findAndModify({
            ...
            projects:req.body.Projects, employeeid: req.body.employeeid,organisation:req.body.organisation,
            designation:req.body.designation, location:req.body.location } },
            new: true
        }, function (err, doc) {
           if (err)
